...
class TaxcomExportController(
    ...
    fun runDiscovery(): ResponseEntity<TaxcomRunResponse> {
        val taxcomExportService = getEnabledService() ?: return exportDisabledResponse()
        taxcomExportService.runDiscoveryAsync()
        return ResponseEntity.accepted().body(TaxcomRunResponse(accepted = true, status = taxcomExportService.status()))
    }

    @PostMapping("/receipts/run")
    fun runReceipts(): ResponseEntity<TaxcomRunResponse> {
        val taxcomExportService = getEnabledService() ?: return exportDisabledResponse()
        taxcomExportService.runReceiptsAsync()
        return ResponseEntity.accepted().body(TaxcomRunResponse(accepted = true, status = taxcomExportService.status()))
    }

    @PostMapping("/discovery/stop")
    fun stopDiscovery(): ResponseEntity<TaxcomRunResponse> {
        val taxcomExportService = getEnabledService() ?: return exportDisabledResponse()
        val accepted = taxcomExportService.stopDiscovery()
        return ResponseEntity.accepted().body(TaxcomRunResponse(accepted = accepted, status = taxcomExportService.status()))
    }

    @PostMapping("/receipts/stop")
    fun stopReceipts(): ResponseEntity<TaxcomRunResponse> {
        val taxcomExportService = getEnabledService() ?: return exportDisabledResponse()
        val accepted = taxcomExportService.stopReceipts()
        return ResponseEntity.accepted().body(TaxcomRunResponse(accepted = accepted, status = taxcomExportService.status()))
    }

    @PostMapping("/stop")
    fun stopAll(): ResponseEntity<TaxcomRunResponse> {
        val taxcomExportService = getEnabledService() ?: return exportDisabledResponse()
        val accepted = taxcomExportService.stopAll()
        return ResponseEntity.accepted().body(TaxcomRunResponse(accepted = accepted, status = taxcomExportService.status()))
    }
