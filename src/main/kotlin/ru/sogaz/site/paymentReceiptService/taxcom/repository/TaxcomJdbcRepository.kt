...
class TaxcomJdbcRepository(
    ...
    fun claimPendingReceipts(limit: Int, maxAttempts: Int, lockTimeoutMinutes: Int, workerId: String): List<TaxcomReceiptRow> {
        val receiptIds = taxcomJdbcTemplate.query(
            """
            ;WITH picked AS (
                SELECT TOP (?) id
                FROM receipts_taxcom WITH (UPDLOCK, READPAST, ROWLOCK)
                WHERE data_uploaded = 0
                  AND load_attempts < ?
                  AND (
                      processing_owner IS NULL
                      OR processing_started_at < DATEADD(MINUTE, ?, SYSUTCDATETIME())
                  )
                ORDER BY date_create
            )
            UPDATE r
            SET processing_owner = ?, processing_started_at = SYSUTCDATETIME(), date_update = SYSUTCDATETIME()
            OUTPUT inserted.id
            FROM receipts_taxcom r
        ...
        return findReceiptsByIds(receiptIds)
    }

    private fun findReceiptsByIds(receiptIds: List<UUID>): List<TaxcomReceiptRow> {
        if (receiptIds.isEmpty()) {
            return emptyList()
        }
        val placeholders = receiptIds.joinToString(",") { "?" }
        return taxcomJdbcTemplate.query(
            """
            SELECT r.id, r.list_shifts_id, k.num_fn, r.tag_1042, r.document_info_uploaded, r.document_url_uploaded, r.subjects_uploaded
            FROM receipts_taxcom r
            JOIN list_shifts s ON s.id = r.list_shifts_id
            JOIN list_kkt k ON k.id = s.list_kkt_id
            ...
            """.trimIndent(),
            { rs, _ -> rs.toReceiptRow() },
            *receiptIds.map { it.toDbId() }.toTypedArray(),
        )
    }
    ...
    fun markReceiptUploadedIfDone(receiptId: UUID) {
        taxcomJdbcTemplate.update(
            ...
            SET data_uploaded = 1, processing_owner = NULL, processing_started_at = NULL,
                date_update = SYSUTCDATETIME(), last_error = NULL
    ...
    fun releaseReceiptProcessing(receiptId: UUID, workerId: String) {
        taxcomJdbcTemplate.update(
            """
            UPDATE receipts_taxcom
            SET processing_owner = NULL, processing_started_at = NULL, date_update = SYSUTCDATETIME()
    ...
    fun saveReceiptError(receiptId: UUID, workerId: String, message: String?) {
        taxcomJdbcTemplate.update(
            """
            UPDATE receipts_taxcom
            SET last_error = ?, load_attempts = load_attempts + 1,
                processing_owner = NULL, processing_started_at = NULL, date_update = SYSUTCDATETIME()
            WHERE id = ? AND processing_owner = ?
            """.trimIndent(),
    ...
    companion object {
        ...
        private val DDL = listOf(
            ...
            CREATE TABLE dbo.receipts_taxcom (
                ...
                processing_owner nvarchar(100) NULL,
                processing_started_at datetime2(6) NULL,
                load_attempts int NOT NULL DEFAULT 0,
                last_error nvarchar(4000) NULL,
                date_create datetime2(6) NOT NULL DEFAULT SYSUTCDATETIME(),
                date_update datetime2(6) NULL,
            ...
            """.trimIndent(),
            """
            IF COL_LENGTH('dbo.receipts_taxcom', 'processing_owner') IS NULL
            ALTER TABLE dbo.receipts_taxcom ADD processing_owner nvarchar(100) NULL
            """.trimIndent(),
            """
            IF COL_LENGTH('dbo.receipts_taxcom', 'processing_started_at') IS NULL
            ALTER TABLE dbo.receipts_taxcom ADD processing_started_at datetime2(6) NULL
            """.trimIndent(),
            """
            IF NOT EXISTS (
                SELECT 1 FROM sys.indexes WHERE name = 'ix_receipts_taxcom_pending' AND object_id = OBJECT_ID('dbo.receipts_taxcom')
            )
            CREATE INDEX ix_receipts_taxcom_pending
            ON dbo.receipts_taxcom (data_uploaded, load_attempts, processing_started_at, date_create)
            INCLUDE (processing_owner)
            """.trimIndent(),
            """
